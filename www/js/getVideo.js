// console.log(Mock.mock);
var data_world = Mock.mock('/world',{
    'list|11': [{
        'url|': '../video/',
        'name|': 'neteaseflvplayerv',
        'index|+1': 1,
        'suffix|': '.mp4'
      }]
});

var data_bilibili = Mock.mock('/bilibili',{
    'b_video_list|1-1':[{
      'title':'侠骨丹心-金庸传',
      'url|':'../video/',
      'file_name|':'xgdx-jyz.mp4',
      'comments|1-1':[{
        'avatar|':'../img/avatar/51.png',
        'user_name|':'sol',
        'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
      },{
        'avatar|':'../img/avatar/23.png',
        'user_name|':'郭靖',
        'comment|':'真正的英雄，要被当事所钦佩，为后人所敬仰，他必定是忧国忧民、怜悯苍生',
      },{
        'avatar|':'../img/avatar/28.png',
        'user_name|':'杨过',
        'comment|':'侠之大者，为国为民',
      },{
        'avatar|':'../img/avatar/50.png',
          'user_name|':'张三丰',
          'comment|':'这正邪二字原本难分，正派弟子心术不正便是邪徒，邪派中人一心向善，便是正人君子。',
      },{
        'avatar|':'../img/avatar/26.png',
          'user_name|':'妲己',
          'comment|':'情义只为初见，',
      }],
      'comments_new|1-1':[{
        'avatar|':'../img/avatar/51.png',
        'user_name|':'匿名网友',
        'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
      },{
        'avatar|':'../img/avatar/23.png',
        'user_name|':'匿名网友',
        'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
      },{
        'avatar|':'../img/avatar/28.png',
        'user_name|':'匿名网友',
        'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
      },{
        'avatar|':'../img/avatar/50.png',
          'user_name|':'匿名网友',
          'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
      },{
        'avatar|':'../img/avatar/26.png',
          'user_name|':'匿名网友',
          'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
      }],
    },{
      'title':'情非泛泛-不得善终',
      'url|':'../video/',
      'file_name|':'qfff-bdsz.mp4',
      'comments|1-1':[{
        'avatar|':'../img/avatar/51.png',
        'user_name|':'圣僧',
        'comment|':'你扪心自问，自从你放弃仙籍，加入万妖之国，你有没有一天开心过？',
      },{
        'avatar|':'../img/avatar/23.png',
        'user_name|':'通背猿猴',
        'comment|':'你们好大胆，竟然将我好朋友雀仙的雕像打烂？好一个孙悟空，我同那雀仙情非泛泛，你竟然叫人将她打烂。',
      },{
        'avatar|':'../img/avatar/28.png',
        'user_name|':'万妖女王',
        'comment|':'在一千年前，你做神猴大将军的时候，你威武不凡，斩妖除魔。在你每一次出战的时候，都有一个小小的雀仙已经对你深深的倾慕。',
      },{
        'avatar|':'../img/avatar/50.png',
          'user_name|':'月老',
          'comment|':'究竟我跟万妖女王日后如何安排？万妖女王同你不得善终。',
      },{
        'avatar|':'../img/avatar/26.png',
          'user_name|':'猪八戒',
          'comment|':'到底改不改？改改改',
      }],
      'comments_new|1-1':[{
        'avatar|':'../img/avatar/51.png',
        'user_name|':'匿名网友',
        'comment|':'真心祝福这对cp，为何结局让人们这么难以接受',
      },{
        'avatar|':'../img/avatar/23.png',
        'user_name|':'匿名网友',
        'comment|':'只要你开心，我做什么都愿意。妹心向明月，明月照沟渠',
      },{
        'avatar|':'../img/avatar/28.png',
        'user_name|':'匿名网友',
        'comment|':'能上苍穹又怎样？',
      },{
        'avatar|':'../img/avatar/50.png',
          'user_name|':'匿名网友',
          'comment|':'唯有你是我的天堂',
      },{
        'avatar|':'../img/avatar/26.png',
          'user_name|':'匿名网友',
          'comment|':'天意注定我偏要反抗',
      }],

    }]
});

var work_list = Mock.mock('/comment',{
  'comment_list|1-1':[{
    'imgUrl|':'../img/avatar/51.png',
    'title|':'再度突破极限，自在极意功登峰造极！',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  }]
    
});

// var comment_new = Mock.mock('/comment-new',{
//   'comment_list|1-1':[{
//     'avatar|':'../img/avatar/51.png',
//     'user_name|':'匿名网友',
//     'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
//   },{
//     'avatar|':'../img/avatar/23.png',
//     'user_name|':'匿名网友',
//     'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
//   },{
//     'avatar|':'../img/avatar/28.png',
//     'user_name|':'匿名网友',
//     'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
//   },{
//     'avatar|':'../img/avatar/50.png',
//       'user_name|':'匿名网友',
//       'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
//   },{
//     'avatar|':'../img/avatar/26.png',
//       'user_name|':'匿名网友',
//       'comment|':'说到金庸的著作，在那些个主角里面，乔峰乔帮主、张无忌张教主、石破天石帮主、杨过、郭靖、令狐冲等等，哲学看过的电视剧。张教主太渣男，说到我个人最欣赏两个人令狐冲和杨过，亦正亦邪，他们两个最为自由这也是我所向往的。',
//   }]
// });

// 音乐列表
var music_list = Mock.mock('/music',{
  'music_list|1-1':[{
    'music|':'../audio/NARUTO_and_HINATA.mp3',
    'title|':'NARUTO and HINATA',
    'author|':'高梨康治',
    'date|':'2019-07-04',
    'text|':'这是《火影忍者》剧场版终章里鸣人和雏田结婚的背景音乐'
  },{
    'music|':'../audio/bring_me_back_to_life.mp3',
    'title|':'Bring Me Back to Life',
    'author|':'Ht Bristol',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/Expelled.mp3',
    'title|':'Expelled',
    'author|':'Brand X Music',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/fireflies.mp3',
    'title|':'fireflies',
    'author|':'Ryan Farish',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/freedom.mp3',
    'title|':'freedom',
    'author|':'Epic Score',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/hongyan.mp3',
    'title|':'hongyun',
    'author|':'高梨康治',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/skyland.mp3',
    'title|':'skyland',
    'author|':'Mich',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/mind_heist.mp3',
    'title|':'mind_heist',
    'author|':'Zack Hemsey',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/wujian.mp3',
    'title|':'wujian',
    'author|':'高梨康治',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/SUPER_EUROBEAT&InitialD&HyperMegaMix&SpaceBoy&remix.mp3',
    'title|':'SUPER_EUROBEAT&InitialD&HyperMegaMix&SpaceBoy&remix',
    'author|':'SUPER EUROBEAT',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/rage.mp3',
    'title|':'rage',
    'author|':'V.A',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/Joy.mp3',
    'title|':'Joy',
    'author|':'勝又隆一',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  },{
    'music|':'../audio/Blizzard.mp3',
    'title|':'Blizzard',
    'author|':'三浦大知',
    'date|':'2019-03-12',
    'text|':'这是说明分析'
  }]
    
});





